...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
      ...
      //register account byl1helper.sol register function
      // deposit 0.1 eth at L1helper.sol wrapandrelaytokens function
    ...
    const tornadoleftBalance =utils.parseEther('0.02')
    expect(tornadoPoolBalance).to.be.equal(tornadoleftBalance)
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
      ...
      expect(omniBridgeBalance).to.be.equal(bobWithdrawAmount)
      
    // withdraws all her remaining funds from the shielded pool
    const aliceWithdrawAmount = utils.parseEther('0.07')
    const aliceEthAddress = '0xca0840578f57fe71599d29375e16783424023357'
